...
export default function SubcategoryForm({ categorySlug, categoryId }: SubcategoryFormProps) {
  ...
  const createSubcategory = apiClient.subcategory.create.useMutation({
    ...
    onError: ({ message }) => {
      toast.error(message || 'An error occurred while creating the subcategory', { id: toastId })
      setToastId('')
      setIsLoading(false)
    },
  })
  ...
  function onSubmit(data: FormValues) {
    setToastId('')
    const id = toast.loading('Creating subcategory')
    setToastId(id)
    setIsLoading(true)
    ...
    >
      <div className="col-span-4 h-full w-full space-y-6">
        <FormSection
          ...
          description="Enter subcategory details"
        >
          <Form.Field
            {...{
              name: 'body.title',
          ...
          <Form.Field
            {...{
              ...
              inlinePrefix: `${env.NEXT_PUBLIC_BASE_URL}/category/${categorySlug}/`,
              required: true,
        ...
        <FormSection
          title="Media"
          description="Upload images and icons for this subcategory"
        >
        ...
        <FormSection
          ...
          description="Configure how this subcategory appears to users"
        >
          <div className="grid grid-cols-1 gap-4 md:grid-cols-2">
