...
export default async function SignUpPage() {
  ...
  return (
    <Shell>
      <AuthCard
        {...metadata}
        footer={<AuthFooterNote hint="Already have an account?" action="Sign in" href={PATH.AUTH.SIGN_IN} />}
      >
        <SignUpForm />
        <div className="relative text-center">
          <span className="text-muted-foreground bg-card relative z-10 px-2 text-xs">or</span>
          <div className="border-border absolute inset-x-0 top-1/2 -translate-y-1/2 border-t" aria-hidden />
        </div>
        <AuthProviders />
      </AuthCard>
    </Shell>
