...
export function useCategoryBulkActions(): BulkAction<CategoryBase>[] {
  ...
  const update = apiClient.category.update.useMutation();
  const del = apiClient.category.delete.useMutation();
  ...
  return [
    {
      ...
      confirmationMessage: (rows) => `Delete ${rows.length} categor${rows.length !== 1 ? "ies" : "y"}?`,
      run: async (rows) => {
    ...
    },
    {
      ...
      confirmationMessage: (rows) => `Make ${rows.length} categor${rows.length !== 1 ? "ies" : "y"} public?`,
      run: async (rows) => {
        await Promise.all(
    ...
    },
    {
      ...
      confirmationMessage: (rows) => `Make ${rows.length} categor${rows.length !== 1 ? "ies" : "y"} private?`,
      run: async (rows) => {
        await Promise.all(
    ...
    },
    {
      ...
      confirmationMessage: (rows) => `Hide ${rows.length} categor${rows.length !== 1 ? "ies" : "y"}?`,
      run: async (rows) => {
        await Promise.all(
    ...
    },
    {
      ...
      confirmationMessage: (rows) => `Mark ${rows.length} categor${rows.length !== 1 ? "ies" : "y"} as featured?`,
      run: async (rows) => {
    ...
    },
    {
      ...
      confirmationMessage: (rows) => `Unmark featured for ${rows.length} categor${rows.length !== 1 ? "ies" : "y"}?`,
      run: async (rows) => {
